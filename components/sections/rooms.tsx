...
function RoomCard({ room, index }: { room: typeof rooms[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <motion.div style={{ scale }} className="relative aspect-[3/4] overflow-hidden mb-6">
        <Image
          src={room.image}
          alt={room.name}
          fill
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 pt-2">
          {room.details.map((detail) => (
            <span key={detail} className="text-primary/70 text-xs tracking-wider uppercase font-sans">
              {detail}
            </span>
          ))}
        </div>
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        >
          ...
          <p className="text-muted-foreground font-sans max-w-md leading-relaxed">
            Each space has been designed with intention, balancing comfort with aesthetic beauty.
          </p>
        </motion.div>
